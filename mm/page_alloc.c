...
//如果释放的是migrate的page是不计入free page的	
					...
					struct per_cpu_pages *pcp)
{
	...
	while (count) {
		 ...
		 * lists
		 */
//此处如果所有的list都是empty，将陷入死循环
//所以调用这个函数之前要检查pcp中有的个数，count不能free大于pcp个数
		do {
			batch_free++;
 ...
 * page in a pageblock.
 */
 //检查start_pfn和end_pfn属于同一个zone，而且都是有效page
 //返回start_pfn的page
 ...
 */
//从zone->free_area[current_order]->free_list[migratetype]中找到最小的order，
//并移除，然后返回给调用者
static inline
 ...
 */
 //将从start_page开始到end_page结束的free的page移动到migratetype的freelist
 //注:只移动在buddy系统中的page
...
//设置migratetype开始的1<<start_order个page的migratetype为migratetype
 ...
 */
 //将page移动到start_type的buddy中，同时设置mt
 ...
 * is not online.
 */
//将zone指定cpu的pcp的page全部释放到buddy系统中
 ...
 */
//将所有cpu的pcp的page释放到buddy系统中
//如果zone!=NULL，则仅仅释放zone的
 ...
 */
 //对于释放order为0的page时，不是直接释放到buddy系统中，而是
 //释放到pcp中，当pcp中的page个数过多时，才统一释放到buddy中
...
int __isolate_free_page(struct page *page, unsigned int order)
{
	...
	zone = page_zone(page);
	mt = get_pageblock_migratetype(page);
//此处需要判断是否是isolate类型，如果是，则不需要进行watermark ok的判断
//因为isolate类型的内存是不能分配的，如果不是isolate类型，则需要进行内存
//watermark的判断，以防止分配之后内存过低
 ...
 */
//从zone中分配order的类型为migratetype的内存， preferred_zone没有用到
static inline
...
//如果order是0的话，从pcp中分配，实际上大部分情况下走的都是这个路径
	if (likely(order == 0)) {
		struct per_cpu_pages *pcp;
		struct list_head *list;
...
//lowmem_reserve是可以通过/proc/sys/vm/lowmem_reserve_ratio设置的
  ...
  //对于order为0的内存，只要freepage的总量够就行了
	if (!order)
		return true;
//对于order大于0的情况，还必须检查是否有大于order的连续内存
...
{
//这个函数仅仅被__alloc_pages_slowpath调用，所以这里设置了ALLOC_WMARK_MIN
//但是作为一个函数，至少命名是有问题的
		     ...
		     bool did_some_progress, int no_progress_loops)
{
	 ...
	 */
//如果连续MAX_RECLAIM_RETRIES回收都没有进展，则不再继续
...
//唤醒内存回收线程开始内存回收
	 ...
	 * that first
	 */
//在gfp_to_alloc_flags中设置了ALLOC_WMARK_MIN
//step 1:直接使用ALLOC_WMARK_MIN尝试分配内存
	page = get_page_from_freelist(gfp_mask, order, alloc_flags, ac);
	if (page)
		goto got_pg;
 ...
 */
 //计算manager_page-high_wmark_pages的值
 //offset表示zone最大的index
 ...
 */
 //计算ZONE_DMA and ZONE_NORMAL区域的除去high watermark的剩余的内存值
 ...
 */
  //计算所有区域的除去high watermark的剩余的内存值
 ...
 */
 //打印很详细的内存信息，包括
 //总内存状态，各个zone的状态，已经各个zone的不同order和type的状态
 ...
 */
 //将pgdat中的zone按照从高到低的顺序添加到zoneref中
...
//计算需要的wait table hash的大小，目前一个hash对应1M的内存
static inline unsigned long wait_table_hash_nr_entries(unsigned long pages)
{
	unsigned long size = 1;
//每个waitqueue对应的内存大小是256*4k=1M
 ...
 */
 //初始化zone的页面为MIGRATE_MOVABLE，同时初始化zone内的所有页面(由memmap管理)
...
//zones_size是数组，里边各个zone的大小已经赋值过了，这里只是计算
//zone_start_pfn和zone_end_pfn
...
//zholes_size是个数字，在其他地方已经计算过了，这里直接返回
...
#endif /* CONFIG_HAVE_MEMBLOCK_NODE_MAP */
//计算pgdat page的数量，并更新zone的信息和pgdat的信息
//在UMA系统中zones_size和zholes_size都已经赋值过了，这个函数其实很简单的
 ...
 */
 //计算总的reserve的page，计算方式看不明白,不明白啥用
 //有一点可以猜测，只要总的free的内存大小大于这个值，无论哪一个zone分配内存
 //都应该能分配到
 //计算方式:sum(max(zone->lowmem_reserve)+high_wmark_pages(zone))
 ...
 */
 //设置lowmem_reserve的大小为managed_pages/sysctl_lowmem_reserve_ratio[idx]
static void setup_per_zone_lowmem_reserve(void)
{
	...
	for_each_online_pgdat(pgdat) {
		for (j = 0; j < MAX_NR_ZONES; j++) {
			...
			idx = j;
//这里可以看出reserve的是从idx向下的，低端内存需要为高端内存reserve，
//这个字段可通过/proc/zoneinfo的protection字段查看，保留内存的大小为
//高端内存的总和除以sysctl_lowmem_reserve_ratio[idx]
//例如如果我们有DMA,NORMAL,MOVABLE三个zone
//则DMA为NORMAL分配保留的内存大小是zone[NORMAL]->managed_pages/sysctl_lowmem_reserve_ratio[DMA]
//DMA为MOVABLE保留的内存大小是
//zone[NORMAL]->managed_pages+zone[MOVABLE]->managed_pages/sysctl_lowmem_reserve_ratio[DMA]
//这里很容易看出如果保留的大小跟高端内存的大小是成正比的关系的，即高端内存越大，保留
//内存也越大，高端内存越小，保留内存也越小，原因是如果高端内存很大，那么高端内存更应该
//通过回收来解决自己分配内存不足的问题，如果高端内存很小，则内存分配时高端内存即使回收
//也无法得到很多的内存，注定是要从低端内存获取的，所以低端内存要给高端内存供应更多的内存。
			while (idx) {
...
static void __setup_per_zone_wmarks(void)
{
	...
	for_each_zone(zone) {
		u64 tmp;
//pages_min表示整个zone watermark[WMARK_MIN]要保留的pages，注，这些page
//只能在低端内存中保留，因为本来就是打算给kernel用的
//每个zone的WMARK_MIN的计算方式是
//pages_min * zone->managed_pages/lowmem_pages，即根据当前zone pages的比例
//类保留适当的页面
		spin_lock_irqsave(&zone->lock, flags);
 ...
 *	changes.
 */
// /proc/sys/vm/min_free_kbytes改变了，重新计算watermark的大小
//这个会更新min watermark的大小，同时low和high也会相应的改变
...
// /proc/sys/vm/watermark_scale_factor改变了，重新计算watermark的大小
//这个会更新low和high watermark的大小
 ...
 */
 //通过/proc/sys/vm/lowmem_reserve_ratio设置lowmem_reserve的大小
 ...
 */
 //通过/proc/sys/vm/percpu_pagelist_fraction设置pcp的high和batch
 //设置pcp->high = zone->managed_pages/percpu_pagelist_fraction和batch
...
//将[start, end)的page migrate出去
//首先如果之间有干净页面，先释放掉，不需要migrate
//对于其他的正在使用的页面调用migrate_pages函数将页面迁移出去
 ...
 */
 //分配从[start,end)的连续内存，中间会有migrate的动作
...
//释放从pfn开始的nr_pages个page，还在使用的页面不会被释放
...
bool is_free_buddy_page(struct page *page)
{
	...
	spin_lock_irqsave(&zone->lock, flags);
//buddy系统仅仅设置了首页的buddy标志，所以要遍历整个order，以确认此page是否在
//buddy系统中
	for (order = 0; order < MAX_ORDER; order++) {
